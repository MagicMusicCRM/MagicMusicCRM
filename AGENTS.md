...
_Актуализация 2026-07-30: `T1.3.1` закрыта: PRD §8 сведён в единый реестр из 53 typed EntityLink-переходов для 13 source-контекстов; 6 ролей получают canonical target либо safe forbidden, unknown=0, filters/date/scroll/column сохраняются. Ad-hoc переходы карточек, задач, reports/audit и dashboard заменены actor-safe navigator; exact matrix + linked UI 24/24, targeted analyze clean (`docs/audits/v4-context-transition-matrix.md`). Для `T1.4.1` подготовлены единый Windows/Android integration harness и S5 gate; локально workspace regression 24/24, reporting/OOXML PostgreSQL 7/7, Flutter reporting/navigation 12/12, fixture реально открыт Microsoft Excel без repair exception. Device execution остаётся открытым: текущий non-elevated Windows не разрешает Developer Mode/plugin symlinks, установку C++ Build Tools и WHPX/AEHD; Android SDK/JDK/API36/NDK/API35 AVD уже установлены. Следующий шаг `/forge`: после elevation выполнить `scripts/v4_workspace_e2e.ps1 -Windows -Android`, закрыть `T1.4.1` и `INT-S5`._
