...
| **Задачи с историей по датам/времени** | ❌ `GetTasks` → 404. 514 задач на проде — тоже из файловой выгрузки. **История нашлась 17.07 — в тексте** выгрузки «Коммуникации»: «(поставил ФИО — дата)» + «Статус "Закрыта" (установил ФИО) — дата время», 12 162 события. Отдельной выгрузки не требуется, разбор — `communications-mappers.ts`. У задач на проде `created_by` пуст у всех 514 (`assigned_to` — наоборот, есть у 513) |
