...
3. **Контекстные экраны копируют доменную функцию.** Клиентский календарь и две воронки развиваются отдельно; задачи уже сведены к единому `SharedTasksPanel`.
...
| Сохранять единым | `SharedTasksPanel` и `app.shared_tasks` | одна задача во всех разделах |
...
| Не возвращать | старый `TasksWidget` и старые write paths | канонический runtime уже использует `SharedTasksPanel` |
...
7. Поддерживать единый `SharedTasksPanel` в dashboard, карточках и уведомлениях; не возвращать legacy task runtime.
