...
## Совместимость резерва для правок заказчика

DECISION (owner, 2026-09-16): готовить выпуск с полной совместимостью резервного
сервера с новой схемой и клиентскими запросами, включая фильтр оплаты преподавателей.
Деградация этого фильтра до HTTP 400 не принимается. Переключение проверяется на
реальных завершённых занятиях, начислениях и XLSX без изменения истории.
Подготовка релиза не является разрешением на deploy. Свежий backup и проверяемый
план восстановления обязательны; общий backport финансовой логики не гарантирует
устранение дефекта в этой же логике при переключении на резерв.
...
DECISION (owner, 2026-09-20): Удалить runtime-интеграцию HolliHop во всём
приложении, сохранив перенесённые данные. Доска лидов использует собственный
справочник направлений и канонические определения клиентских полей для уровней
и категорий. Серверные HolliHop-прокси и их сетевой сервис удалены. Контакты,
исходные даты, предыдущие статусы, идентификаторы происхождения и история
продолжают читаться; миграции и средства восстановления импорта сохранены.
Изменение исходников не означает deployment.
DECISION (owner, 2026-09-20): Выпуск этих изменений в production отложен до
внесения остальных запланированных изменений. При выпуске обновлять клиент и
сервер согласованно: прежний клиент использует удалённые metadata endpoints.
