...
По прямой команде владельца опубликовано исправление проверки рабочего графика
преподавателя при создании и изменении занятия и повторяющегося расписания.
Если график преподавателя не задан, запись разрешена в часы филиала.
Source/tag: `eb2575d30f24472c5500bb921dddc30b659a2286` / `v1.5.50`.

API image `sha256:2f08d092013f01f7b761fc714dd5033b96619b2d01813f0080109d262725f874`,
схема `0161_restore_notification_preferences`, readiness `ok`, outbox `0/0`,
health healthy/restart 0. Оба update-канала и история показывают build 230;
Windows Setup/ZIP и подписанные APK/AAB опубликованы. Полный backend gate:
4166 PASS; Flutter: 1881 PASS, 4 штатных skip; production-like UI/HTTP/restore:
24 PASS. Pre/post encrypted backups скопированы вне сервера и восстановлены
в изоляции на новом и резервном образах. Подробности:
[release 230 audit](../audits/release-230-production.md).

Ручная визуальная приёмка с реальными ролями и данными заказчика не выполнялась;
релиз сам по себе не означает полную приёмку всего ТЗ.
...
По прямому разрешению владельца опубликован клиентский выпуск с непрерывной
карточкой клиента, компактными основными полями и ранним показом ленты,
коротким пустым блоком задач, тремя начальными событиями истории, насыщенными
цветами списания и одной рамкой/полной подсказкой занятия без дублирующих значков.
Source/tag: `c9e7db1e69f09053ea6ed0b57286987c1481de46` / `v1.5.46`.

Windows Setup/ZIP, подписанные APK/AAB, оба update-канала и GitHub Release
проверены по размерам и SHA-256. Полный backend: 4158 PASS; production-like
UI/HTTP/restore: 24 PASS. Flutter: 1861 PASS + исправление единственной ошибки
текста релиза с повторными 10 PASS; updater: 4 PASS. Windows/Android Release
launch smoke пройден. Подробные границы и неуспешные промежуточные прогоны:
[release 226 audit](../audits/release-226-production.md).

API и БД не переключались: image `magicmusiccrm-server:1.5.44-224-final`,
schema `0160_requeue_lead_create_outbox`. После публикации readiness OK,
outbox `0/0`, reconciliation `issues=[]`. Свежий encrypted backup
`20260922T184222Z` скопирован вне сервера и восстановлен в изоляции — PASS.
Rollback manifest build 225 сохранены; финансовую/учебную/audit-историю
не откатывать. Следующий шаг владельца — ручная визуальная приёмка build 226;
выпуск не является заявлением о полной приёмке всех 20 требований.

## История локальной переработки карточки — 22.09.2026, вошла в 226

После ручной проверки владелец обнаружил чрезмерную высоту основных полей,
пустых задач и истории, бледные цвета и вложенные рамки/подсказки занятий.
Эта обратная связь отменяет прежний вывод о достаточной плотности. Исправления
и новая проверка с полным набором полей описаны в
`docs/audits/client-card-density-2026-09-22.md`. На этом этапе это был локальный
кандидат, не production-релиз и не подтверждение полной приёмки 20 пунктов.

Владелец отклонил секционную desktop-компоновку build 225. Новое требование:
редактируемые контакты и рабочие действия слева; заметка и основные поля сверху;
все рабочие блоки на общей странице; карандаш редактирования ФИО в заголовке.
Изменения выполняются поверх `d76b7ba39b895e713d27987be4900d2cee509ae2` в
`codex/adaptive-window-layout`, с сохранением исходных чужих изменений.

Клиентская реализация и её ограничения описаны в
[локальном отчёте](../audits/client-card-continuous-2026-09-22.md).
На локальном этапе production, API, схема, update-каналы и версия приложения
не изменялись. После отдельной команды владельца выполнены проверки и выпуск
226, описанные выше. Предыдущие release PASS сами по себе этот кандидат
не подтверждали.

Кнопка «Добавить лид» пока не реализована: владельцу задан вопрос, означает ли
она новое обращение этого клиента или создание отдельного клиента. Существующий
LeadCreateDialog создаёт самостоятельного лида; автоматически дублировать
текущую карточку нельзя. Эта неопределённость не блокирует остальную переработку.
