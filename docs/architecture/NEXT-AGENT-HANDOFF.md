...
## Локальные изменения после 216 — ещё не опубликованы

Исправлена цепочка замены абонемента: лента учитывает перенесённые резервы,
общий allocator подбирает ближайшие занятия из предшествующих контрактов,
возврат к большему пакету восстанавливает освобождённые резервы. Планирование,
расчёт и его повтор учитывают актуальный контракт без переписывания старых
снимков/решений/фактов. Воспроизведение и проверки: dist/funding-archive217/.

Добавлены preview/commit архивирования завершённых индивидуальных планов,
кнопка «В архив», объяснения blockers, обязательная причина и свёрнутый «Архив».
Только отменённые занятия без оплат/ненулевых фактов и резервов можно скрыть.
Миграция 0155_schedule_plan_archive добавляет архивные метки и защищает занятия
от новых финансовых операций и возврата в действующий статус. История остаётся.

Номер версии не менялся, установщик не собирался; production/данные не менялись.
Для выпуска нужны полный gate нового кандидата, свежий backup/restore drill,
сервер с 0155 до клиента (новый includeArchived несовместим с API 216), затем
preview общей утилиты reconcile-subscription-coverage для ранее сломанных резервов.
Её apply допустим только в авторизованном выпуске после проверки результата.
После первой архивации старый сервер не понимает метки; нужен совместимый
forward fix. Down-миграция запрещает потерю сохранённой архивной истории.
Отдельная очередь продления по-прежнему не реализована.
