...
## Изменение настройки входа 2026-09-09

По прямому решению владельца один системный администратор добавлен в production
`AUTH_OTP_BYPASS_EMAILS`: вход по проверенному паролю без обязательного email OTP.
Состав остальных исключений и прочие настройки сохранены; runtime/image 212 прежний.
Персональные сведения остаются только в закрытой серверной конфигурации.
38 auth-тестов PASS; runtime-проверка аккаунта, readiness и reconciliation PASS.
Backup `20260909T165945Z` сохранён off-host, checksum совпал. Локальные результаты:
`dist/otp-account-20260909/`. Серверный журнал и rollback настройки с тем же image:
`/opt/magicmusiccrm/operations/otp-20260909T170144Z-00b018/`.
## Локальные UI-исправления 2026-09-09 — не опубликованы

По запросу владельца сетки дня/недели продолжаются до 01:00 следующего дня с
прежним масштабом 08:00–22:00. После полуночи сохраняется фактическая дата;
дневная загрузка дополнена данными следующего дня. Общий модальный контейнер
получил более свободные отступы и кнопки, ссылка занятия на себя удалена.
Редактор и расчёт показывают ошибки у обязательных полей, фокусируют первое
ошибочное поле и прокручивают форму к ошибке или серверному объяснению.

136 релевантных Flutter-тестов PASS, `flutter analyze --no-pub lib test` без
замечаний; Windows Release собрана. Это локальная проверочная сборка на базе
1.5.32+212, а не новый выпуск. Копия: `dist/pc-preview-ui-comfort-20260909/`;
логи, снимок карточки на синтетических данных и контрольные суммы:
`dist/ui-comfort-20260909/`. Production и опубликованные артефакты не изменены.

Уточнение после локальной проверки владельцем: у синхронных заголовков и шкалы
времени отключены автоматические desktop-scrollbar. Сетка показывает по одному
управляющему скроллбару на ось. 24 календарных теста и отдельный визуальный прогон
PASS, анализ изменённых файлов без замечаний; Windows Release пересобрана.
Актуальная локальная копия: `dist/pc-preview-ui-comfort-20260909-scrollbar/`.
