# MagicMusicCRM — актуальная передача 227

## Production 1.5.47+227 — выпущен 24.09.2026 (Europe/Moscow)

По прямому поручению владельца опубликованы API и Windows/Android пакеты из
`19de8f4017a75885e2b1bca6dce2c03b41c8752f` / `v1.5.47`. API image
`magicmusiccrm-server:1.5.47-227-final`, схема остаётся
`0160_requeue_lead_create_outbox`. Оба update-канала показывают build 227;
четыре публичных пакета, readiness, reconciliation и pre/post encrypted backup
restore проверены. Подробные SHA-256, gates и rollback:
[release 227 audit](../audits/release-227-production.md).

Функция «Добавить лид» исключена из ТЗ прямым решением владельца. Полная
authenticated UAT-приёмка 20 требований ещё не проведена; release-проверки не
служат её заменой. Следующий этап — сценарная проверка установленного build 227
под рабочими ролями с чтением сохранённых данных.
...
Историческая идея кнопки «Добавить лид» не является критерием приёмки:
владелец позднее прямо исключил её из ТЗ.
