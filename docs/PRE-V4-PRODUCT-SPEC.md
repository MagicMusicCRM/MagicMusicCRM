# MagicMusicCRM — единая продуктовая спецификация правок перед v4

| Поле | Значение |
|---|---|
| Статус | **Review — на продуктовом согласовании** |
...
| Назначение | Зафиксировать полный продуктовый объём до создания `.anws/v4`, архитектуры и технических эпиков |

> Этот файл ещё не является `.anws/v4/01_PRD.md`. После продуктового апрува его требования становятся входом для v4, архитектурных решений и технической декомпозиции. До апрува технические эпики и задачи не создаются.
...
## 15. Gate продуктового апрува

На момент передачи:

- Открытые продуктовые вопросы: **нет**.
- Технические эпики: **не созданы**.
- `.anws/v4`: **не создана**.
- Статус требований: **ожидают апрува владельца продукта**.

После явного апрува этого файла следующий этап:

1. Создать `.anws/v4` по проектному workflow.
2. Сформировать v4 PRD на основе зафиксированных REQ без изменения смысла.
3. Разработать архитектуру, модели данных и ADR.
4. Разбить продуктовые блоки на технические эпики и задачи.
5. Провести отдельный review технического backlog по трассировке к этому документу.
