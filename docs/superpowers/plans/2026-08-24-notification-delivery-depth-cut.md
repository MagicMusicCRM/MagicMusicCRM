...
**Goal:** Separate notification HTTP composition from the durable delivery runtime and complete the Sentrux depth reduction from 13 to 12.
...
- Acceptance requires Sentrux `depth.raw <= 12`, `quality_signal >= 4975`, no new cycle, and passing architectural rules.
...
- `server/src/notifications/notifications.module.ts`: HTTP controllers and guards only; re-exports delivery.
...
delivery providers and the direct `AuditModule`/`DatabaseModule` imports:
...
no new cycle, and no quality regression. Depth can remain 13 until consumers
stop importing the API shell.
