...
- **UAT-065:** купить рассрочный пакет: резервируется вся сумма, формируются части,
  оплата части создаёт `Проведён, ожидает подтверждения` с пометкой проверки,
...
- **UAT-072:** создать групповой Plan и проверить участников/абонементы каждой
  строки.
- **UAT-073:** обязательные поля и конфликты проверяются до последних экранов;
...
- **UAT-076:** календарь клиента: `Скрывать чужие занятия` включён по умолчанию;
  после выключения чужие серые, занятия клиента зелёные. Month окрашивает дни,
...
- **UAT-085:** воспроизвести безопасную ошибку плана: состояние становится
  `Ожидает расчёта`, ошибка видна staff и устраняется через preview/confirm.
- **UAT-086:** выполнить post-completion correction атомарно; старая история не
  переписывается, добавляется новая версия с причиной.
...
- **UAT-092:** проверить student/teacher/room overlap, часы филиала, закрытый день,
  недоступность преподавателя, чужой филиал и пересечение по длительности.
...
- **UAT-145:** архивировать тестового клиента только после impact preview и
  проверки запрета для non-Director; проверить сохранность conversion/finance
  links.
