...
| Переход из карточки правильного связанного лида переиспользует ту же вкладку и включает `Скрывать чужие занятия` (историческое поведение `+175`; зелёная заливка target заменена в `+180` независимым маркером связи) | PASS | `windows-release/45-client-filtered-canonical-schedule-175.png` |
