...
try {
    ...
    npm --prefix server run contract:payments:check
    if ($LASTEXITCODE -ne 0) { throw 'Payment OpenAPI drift check failed.' }
    flutter test --no-pub test/core/api/expense_contract_test.dart test/core/api/payment_contract_test.dart test/core/api/payment_adjustment_contract_test.dart --reporter expanded
    if ($LASTEXITCODE -ne 0) { throw 'Flutter wire contract failed.' }
    Push-Location server
    try {
        $testPaths = @('src/contracts/expense-contract.spec.ts', 'src/contracts/payment-contract.spec.ts', 'src/crm/commerce/payment-result-consistency.spec.ts')
        if (-not $Offline) { $testPaths += @('src/contracts/expense-http-postgres.integration.spec.ts', 'src/contracts/payment-http-postgres.integration.spec.ts') }
...
else { Write-Output 'PASS: OpenAPI, Flutter wire fixtures and HTTP/PostgreSQL expense and payment contracts.' }
