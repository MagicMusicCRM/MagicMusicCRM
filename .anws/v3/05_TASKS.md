...
- [ ] **INT-S6** [MILESTONE]: Flutter v3 smoke
  ...
  - **Status**: Partially verified on 2026-06-12 against `https://api.phantom-net.ru/api`. `flutter doctor -v` is clean, Android licenses are accepted, `flutter build apk --debug --dart-define=MAGIC_API_BASE_URL=https://api.phantom-net.ru/api` passed and produced `build/app/outputs/flutter-apk/app-debug.apk`; `flutter build windows --debug --dart-define=MAGIC_API_BASE_URL=https://api.phantom-net.ru/api` passed and produced `build/windows/x64/runner/Debug/magic_music_crm.exe`; Windows runtime smoke kept the app alive for 20 seconds from the debug exe. Real Android device `I2405` on Android 15 was connected over USB and passed UI smoke through signup-created staging account login, onboarding profile form, legal consent gate, dashboard chat list, Administration chat open and message send. The sent Android UI message `AndroidSmokeMessage` was visible in-app and confirmed through v3 API in chat `247e9015-d125-41c3-86c9-a8ba9aa580e8` as message `f73a5583-14f5-42b5-901e-a9c472e3dd8e`. Device logcat had no Flutter/Dart/Fatal app errors; earlier Android frame samples for login/onboarding/dashboard had jank around `1-2%` with p95 UI frame time `14-15ms`.
    ...
    Added `integration_test/app_launch_smoke_test.dart` automation and `docs/runbooks/flutter-integration-smoke.md`: the no-secret smoke starts the app with an in-memory token store/no-op notifications, reaches the Russian login gate, validates empty login errors, starts an authenticated fake account-deletion route, verifies `Отправить запрос` is disabled until acknowledgement, submits the fake request and reaches `Запрос принят`. Added `docs/runbooks/android-real-device-smoke.md` and `scripts/android_real_device_smoke.ps1` for the remaining stable-device private file, real-backend deletion, Phase 04-10 workflow, log evidence and cleanup checklist; `-CheckOnly` passed locally. The Windows runner passed the integration smoke with `2` tests. Remaining gap: run the integration smoke/helper on a stable Android target, and real Android UI smoke still needs private file upload/download plus account deletion against the real backend before INT-S6 can close.
...
- [x] **T7.1** [REQ-V3-SEC-001]: Complete security checklist
  ...
  - **Estimate**: 12 h.
  - **Status**: Completed as a pre-release gate on 2026-06-12. Codex Security repository pass produced `C:\tmp\codex-security-scans\MagicMusicCRM\c683807_20260612T204722\report.md` and `report.html`; official report validator passed. High findings found during the pass were remediated in working tree: `server/exports` and `server/storage` are now excluded from Git and Docker context, chat attachment reads now require chat membership, and login/OTP verify now have audit-backed failed-attempt lockouts. Release evidence is recorded in `.anws/v3/09_S7_RELEASE_EVIDENCE.md`. Production-only follow-ups remain: rotate previously exposed credentials, run history-aware secret scan, complete external SAST/container scans, capture final host hardening evidence, and complete user-owned Android UI smoke for files/account deletion.
...
- [ ] **T7.3** [REQ-V3-MIG-001]: Execute production cutover rehearsal
  - **Description**: Practice freeze/export/import/smoke/rollback steps on staging.
  - **Verification**: Rehearsal report accepted.
  - **Estimate**: 12 h.
  - **Status**: Deferred on 2026-06-12. Staging migration/export/import, file migration and realtime smoke have passed, but final production rehearsal requires the final production server, final DNS target and rotated credentials. Local rehearsal prep advanced on 2026-06-15: added `scripts/hollihop_staging_dry_run.ps1`, `docs/runbooks/hollihop-staging-dry-run.md` and importer docs coverage for the HolliHop Phase 03 DB-backed staging dry-run. The helper forces `HOLLIHOP_IMPORT_MODE=dry_run`, passes `--dry-run`, requires backup evidence, writes reports to Supergoal evidence, hides DB/HolliHop secret values, refuses pre-set `apply` mode, safely loads ignored local env files and passed `-CheckOnly`. Archive DB-backed staging dry-run passed after encrypted backup `magicmusiccrm-staging-20260615T131610Z.tgz.enc`: report `hollihop-import-2026-06-15T13-40-42-091Z.json`, batch `3c4fc480-74a7-4801-a0e2-45c26972004a`, warnings `tasks_source_missing` and `timeline_sources_missing`, secret grep clean. Live HolliHop validate-only inventory also passed with `1,025` students, `1,944` leads and `3,166` payments. Remaining rehearsal blocker: live DB-backed `-UseLiveApi` dry-run fails at the connection/client stage before report generation.

- [ ] **T7.4** [REQ-V3-MIG-001]: Execute production cutover
  - **Description**: Freeze Supabase writes, run final migration, switch DNS/API and monitor.
  - **Verification**: Production smoke passes and alerts remain clean.
  - **Estimate**: 8 h.
  - **Status**: Deferred on 2026-06-12 until production `api.magic-music.org` is pointed to the final v3 backend and the user approves the final write freeze/cutover window.

- [ ] **INT-S7** [MILESTONE]: Launch acceptance
  ...
  - **Estimate**: 4 h.
  - **Status**: Pre-release acceptance completed on 2026-06-12 for Google Play AAB `v1.1.6+116`; full production launch acceptance remains open until T7.3/T7.4 and user-owned S6 Android file/deletion smoke are complete.
