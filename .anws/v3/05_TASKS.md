...
- [x] **T9.1** [REQ-V3-DATA-001, REQ-V3-MSG-001]: Close the demo-critical lead, homework, subscription and billing flow
  ...
  - **Estimate**: 32 h.
  - **Status**: Completed on 2026-07-18. Migration `0072_demo_workflow_invariants` is applied in production and API health is green. The lead remains a lead through trial/homework and converts atomically only when a subscription is issued; ordinary attendance charges once while trial attendance does not charge. Real PUSH delivery was observed. Verification passed: backend `96/96` suites and `907/907` tests plus typecheck/build; Flutter `flutter analyze` clean and `394/394` tests.

- [x] **T9.2** [REQ-V3-OPS-001, REQ-V3-DATA-001]: Add a guarded, repeatable demo reset and preflight
  ...
  - **Estimate**: 16 h.
  - **Status**: Completed on 2026-07-18. The allowlisted `magic1@gmail.com` reset ran against production only after a verified backup; guarded post-check confirmed the demo CRM graph and private chat data were clean while shared/account state remained intact.

- [x] **T9.3** [REQ-V3-SEC-001]: Automate the four Android role sessions
  ...
  - **Estimate**: 24 h.
  - **Status**: Completed on 2026-07-18. The project-local runner binds the exact `Client`, `Teacher`, `Admin` and `Manager` AVDs, verifies package `magic.crm` version `1.2.2+145`, starts isolated Appium sessions on ports `8211`-`8214`, authenticates the four supplied accounts on their matching devices and supports dry-run, bounded runs, resume/reconcile and five-second proof holds. Runner tests pass `14/14`; the full scenario dry-run passes with `43` automated business/UI steps plus one guarded operator reset checkpoint. A non-mutating four-role login rehearsal passed so the clean production fixture remains untouched for the OBS recording.

- [x] **T9.4** [REQ-V3-DATA-001]: Publish the executable four-role demo scenario
  ...
  - **Estimate**: 8 h.
  - **Status**: Completed on 2026-07-18. The approved lead → trial → homework/feedback → subscription conversion → recurring lesson → one-time charge story is published in `docs/demo/FOUR-ROLE-DEMO-WORKFLOW.md`; signed APK and AAB `1.2.2+145` were produced for the demo release.

- [x] **INT-S9** [MILESTONE]: Four-role Android demo acceptance
...
- [x] **T9.5** [REQ-V3-DATA-001, REQ-V3-OPS-001]: Preserve pending lead edits during subscription conversion and publish the corrected release
  ...
  - **Estimate**: 8 h.
  - **Status**: Completed on 2026-07-19. Client build `1.2.2+147` persists pending lead edits before issuing the first subscription, and regression coverage proves the lead PATCH precedes conversion. Migration `0073_remove_rejected_crm_fields` is applied on production (`73/73`), removes `workplace`, `position` and `individualPrice` from the configured CRM schema while leaving historical student custom data untouched, and production health is green. Flutter analyze and `394/394` tests pass; backend typecheck, build and `98/98` suites (`913/913` tests) pass. Setup, Windows ZIP, signed APK and AAB are stored in the root `dist`, published with verified SHA-256, and both `latest.json` and `latest-v2.json` point to build `147`.

- [x] **T9.6** [REQ-V3-AUTH-001, REQ-V3-MSG-001, REQ-V3-FILES-001, REQ-V3-OPS-001]: Stabilize registration, repeated login and mobile chat attachments
  - **Description**: Make the registration phone input self-explanatory with a persistent `Номер телефона` label and an empty-state Russian mask. Prevent the attachment confirmation dialog from auto-opening or being displaced by the software keyboard, await the upload/send result and surface failures. Support image message attachments end to end, and keep the client-facing administration conversation titled `Администрация` instead of exposing an assigned staff profile. Remove the pre-request local-session reset that races login routing while preserving fail-closed session clearing for OTP challenges. Configure audited OTP bypass only for `magic1@gmail.com` through `magic5@gmail.com`.
  - **Verification**: Flutter tests cover phone label/mask, keyboard-safe attachment layout, awaited success/error behavior, administration-title mapping, no signed-out emission during a successful password login and successful login after logout. Backend tests accept image messages only with normal attachment authorization. Production OTP configuration names exactly five approved demo accounts; backend deploy/health and attachment smoke pass. Setup, Windows ZIP, signed APK and AAB for build `1.2.2+148` are published with matching SHA-256 and both update manifests point to build `148`.
  - **Estimate**: 12 h.
  - **Status**: Completed on 2026-07-19. Registration now keeps a visible phone label and Russian mask; attachment confirmation is keyboard-safe, awaits upload/send and retains actionable errors; client administration chats keep the system identity. Password login no longer emits an intermediate signed-out state, including relogin after logout. Production OTP bypass contains exactly `magic1@gmail.com` through `magic5@gmail.com`. Backend migration `0074_image_message_type` is applied and real production smoke proved direct-chat creation, JPEG upload, `image` message delivery, receiver visibility and authorized download before cleanup. Flutter analyze and `400/400` tests pass; backend typecheck/build and `100/100` suites (`918/918` tests) pass. Build `1.2.2+148` Setup, Windows ZIP, signed APK and AAB are in the root `dist`, all public URLs return HTTP 200, and both manifests resolve to build `148` with the verified Windows ZIP SHA-256.
