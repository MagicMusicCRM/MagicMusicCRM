...
## 2026-08-03 — Owner hotfix: поиск, расписание и drilldown отчётов

- [FIX] Добавлена повторная приёмка серверного поиска: debounce не имеет права заменять загруженный экран skeleton/spinner или терять focus/text; применяется latest-wins.
- [CHANGE] Рабочие режимы расписания зафиксированы как `Месяц / Неделя / День`; годовой режим удаляется.
- [FIX] Переход из карточки клиента открывает месячное расписание с фильтром конкретного Lead/Student, а не произвольный день ближайшего занятия.
- [FIX] Report drilldown обязан вызывать существующий `/analytics/v4/client-status/clients`; backend contract не меняется.
- [ADD] Создана owner hotfix wave T1.5.1/T4.3.4/T7.2.3/INT-HF1 на основании прямого подтверждения владельца.
