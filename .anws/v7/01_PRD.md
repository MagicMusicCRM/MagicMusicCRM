...
- **Описание:** Как сотрудник, я хочу вручную выбрать способ оплаты
  преподавателю для каждого расчётного действия, чтобы контролировать выплату.
...
- **Критерии приемки:**
  ...
  - Сотрудник всегда выбирает тип отдельно от клиентского списания и видит итог.
  - Директор задаёт default value; изменение итогового процента/суммы сотрудником
    требует причины.
  - Начисление фиксируется после terminal lesson transition независимо от
    статуса оплаты клиента и не меняется задним числом.
