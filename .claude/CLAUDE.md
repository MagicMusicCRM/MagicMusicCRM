...
Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-09-16 (commit f6b7806ff). Confidence: 100%.
...
- `lib/features/admin/presentation/widgets/create_lesson_dialog.dart` — 22 bug fixes, last fix 3 days ago (bug magnet); 17 commits/90d
- `server/src/crm/schedule/schedule-plan-postgres.integration.spec.ts` — 17 bug fixes, last fix 11 days ago (bug magnet); 27 commits/90d
- `server/src/crm/crm.service.ts` — 28 bug fixes, last fix 3 days ago (bug magnet); 8 commits/90d
- `lib/features/crm/presentation/client_card/client_card.dart` — 17 bug fixes, last fix 3 days ago (bug magnet); 18 commits/90d
- `server/src/crm/crm.service.spec.ts` — 23 bug fixes, last fix 3 days ago (bug magnet); 13 commits/90d

### Code health
Three co-equal signals: defect risk 7.33/10 avg, hotspot health 5.36/10 (stable), worst `lib/core/router/app_router.dart` at 1.0/10 · maintainability 8.46/10 · performance risk 111 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.
...
- `infra/scripts/deploy-api-release.sh` — complex conditional (automatic_rollback) — impact −2.5
- `server/src/crm/commerce/subscription-preview-token.ts` — complex conditional (assertExactPayload) — impact −2.5
