-- Optional fact columns are deliberately retained to preserve immutable history.
-- Reject rollback after cross-account facts exist: the old projection cannot represent them.
do $$ begin
  if exists (select 1 from app.lesson_client_charge_facts where payer_student_id is not null and payer_student_id <> client_id) then
    raise exception 'Cannot roll back lesson payer projection while cross-account lesson history exists';
  end if;
  if exists (
    select 1 from app.lesson_settlement_plans plan,
      jsonb_array_elements(coalesce(plan.decision->'clientDecisions', '[]'::jsonb)) choice
    where plan.state in ('planned', 'review_required')
      and choice ?| array['chargeType', 'basePriceMinor', 'discount', 'surcharge']
  ) then
    raise exception 'Cannot roll back lesson funding while explicit funding plans are pending';
  end if;
end $$;
