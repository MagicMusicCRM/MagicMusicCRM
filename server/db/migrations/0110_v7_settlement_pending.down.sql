...
begin
  if exists (
    select 1 from app.lessons where lifecycle_state = 'settlement_pending'
  ) or exists (
    select 1 from app.lesson_transitions
    where from_state = 'settlement_pending'
  ) or exists (
    select 1 from app.lesson_completion_work
    where terminal_state = 'settlement_pending'
  ) then
    raise exception
      'Refusing destructive rollback: settlement-pending lesson facts exist';
  end if;
end $$;
