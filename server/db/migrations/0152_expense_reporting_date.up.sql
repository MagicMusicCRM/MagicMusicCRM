-- Rebuild derived reporting data; preserve source expense history.
drop materialized view if exists app.mv_finance_monthly;
create materialized view app.mv_finance_monthly as
with bounds as (
  select least(
    coalesce((select min(date_trunc('month',scheduled_at)) from app.lessons where deleted_at is null),date_trunc('month',now())),
    coalesce((select min(date_trunc('month',payment_date)) from app.payments where deleted_at is null),date_trunc('month',now())),
    coalesce((select min(date_trunc('month',coalesce(occurred_at,created_at))) from app.expenses where deleted_at is null),date_trunc('month',now()))
  ) as min_month
),
months as (
...
expense_stats as (
  select date_trunc('month', coalesce(occurred_at,created_at))::date as m, sum(amount) as expenses
