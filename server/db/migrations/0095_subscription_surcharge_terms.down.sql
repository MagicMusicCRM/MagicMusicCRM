...
begin
  if exists (
    select 1 from app.subscriptions where surcharge_minor is not null
  ) then
    raise exception 'cannot remove subscription surcharge terms while facts exist';
  end if;
end;
$$;
...
alter table app.subscriptions
  drop column if exists surcharge_reason,
  drop column if exists surcharge_minor;
        ...
        or (
          ...
          and final_price_minor = greatest(
            ...
            base_price_minor - round(
              base_price_minor::numeric * discount_percent_basis_points / 10000
            )::bigint
          )
        )
        or (
