...
with monetary_facts as (
  ...
  select
    ...
    0::numeric,
    0::numeric,
    case when obligation.direction = 'debit'
      then obligation.amount_minor::numeric else 0::numeric end,
    case when obligation.direction = 'credit'
      then obligation.amount_minor::numeric else 0::numeric end,
    0::numeric,
    0::numeric,
    0::numeric
...
create or replace view app.commerce_receivable_schedule_projection
with (security_invoker = true) as
select
  record.id as source_id,
  'payment_record'::text as source_kind,
  record.student_id,
  record.due_at,
  record.amount_minor,
  record.currency_code,
  record.status
from app.commerce_ordinary_payment_records record
where record.status in ('unpaid', 'posted_pending')
  and record.due_at is not null

union all

select
  installment.id as source_id,
  'installment'::text as source_kind,
  coalesce(subscription.payer_student_id, subscription.student_id) as student_id,
  installment.due_at,
...
begin
  if exists (select 1 from pg_roles where rolname = 'magiccrm_app') then
    ...
    grant select on app.commerce_receivable_schedule_projection to magiccrm_app;
  end if;
end $$;
