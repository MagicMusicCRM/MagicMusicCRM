-- Migration 0140 was already deployed before its repair function initialized
-- aggregate_versions. Redefine the function under a new migration id so
-- upgraded databases and fresh installs converge without rewriting history.
...
begin
  if exists (
    ...
    raise exception
      'legacy subscription aggregate version reconciliation failed after repair';
  end if;
end $$;
