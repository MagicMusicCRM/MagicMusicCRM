-- Preserve lesson recipients while debiting the explicitly selected student payer.
alter table app.lesson_client_charge_facts
  add column if not exists payer_student_id uuid references app.students(id) on delete restrict,
  add column if not exists pricing_snapshot jsonb;

do $$ begin
  if not exists (select 1 from pg_constraint where conrelid = 'app.lesson_client_charge_facts'::regclass and conname = 'lesson_charge_pricing_snapshot_object') then
    alter table app.lesson_client_charge_facts add constraint lesson_charge_pricing_snapshot_object check (pricing_snapshot is null or jsonb_typeof(pricing_snapshot) = 'object');
  end if;
end $$;

create or replace view app.lesson_client_charge_facts_effective as
select fact.* from app.lesson_client_charge_facts fact
where not exists (select 1 from app.lesson_client_charge_facts newer where newer.supersedes_fact_id = fact.id);
