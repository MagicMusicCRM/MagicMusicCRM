...
describe('NotificationWorker', () => {
  it('falls back to SMTP when primary email provider fails', async () => {
    ...
    const resend = {
      send: jest.fn().mockResolvedValue({ provider: 'resend', status: 'failed', error: 'status_500' })
    };
    const smtp = {
      send: jest.fn().mockResolvedValue({ provider: 'smtp_fallback', status: 'sent' })
    };
    ...
    await expect(worker.dispatchPendingEmails()).resolves.toEqual({ processed: 1, failed: 0 });
  ...
  it('marks outbox failed and audits when both email providers fail', async () => {
    ...
    const audit = { record: jest.fn().mockResolvedValue(undefined) };
    const resend = {
      send: jest.fn().mockResolvedValue({ provider: 'resend', status: 'failed', error: 'timeout' })
    };
    const smtp = {
      send: jest
        .fn()
        .mockResolvedValue({ provider: 'smtp_fallback', status: 'failed', error: 'smtp_timeout' })
    };
    const tokenCrypto = { decrypt: jest.fn() };
    const pushProvider = { send: jest.fn() };
    const worker = new NotificationWorker(
    ...
    await expect(worker.dispatchPendingEmails()).resolves.toEqual({ processed: 1, failed: 1 });
  ...
  it('sends queued push delivery to enabled devices', async () => {
    const query = jest
      ...
      .mockResolvedValue({ rows: [] });
    const audit = { record: jest.fn().mockResolvedValue(undefined) };
    const tokenCrypto = { decrypt: jest.fn().mockReturnValue('push-token-1234567890') };
    const pushProvider = {
      send: jest.fn().mockResolvedValue({ provider: 'firebase', status: 'sent' })
    };
    const worker = new NotificationWorker(
      { query } as unknown as DatabaseService,
      audit as unknown as AuditService,
    ...
    await expect(worker.dispatchPendingPush()).resolves.toEqual({ processed: 1, failed: 0 });
    ...
    expect(query).toHaveBeenCalledWith(expect.stringContaining('update app.notification_deliveries'), [
      'delivery-a',
      'firebase',
      'sent',
      null
    ...
    expect(audit.record).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        metadata: { provider: 'firebase', status: 'sent', deviceId: 'device-a' }
      })
    );
  });
