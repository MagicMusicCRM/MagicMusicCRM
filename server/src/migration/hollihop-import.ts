...
//
// Главный импортёр HolliHop: справочники, педагоги, персонал, лиды, ученики,
// группы, занятия, посещаемость, платежи, история статусов лида.
//
// ИСТОРИЯ ФАЙЛА. Его удалили в 7f2a3fd7 («импорт завершён и сверен, разовая
// оснастка — мёртвый груз»). Решение было разумным ровно до того дня, когда
// базу понадобилось пересобрать с нуля, — и тогда выяснилось, что заливать её
// нечем. Возвращён из 7f2a3fd7^ 17.07.
//
// ЧЕМ ОН ВЛАДЕЕТ. Сущностями. Эту часть можно проверить и она проверена: её
// выход на проде сходится — 1977 лидов, 1036 учеников, 33 088 занятий, 3 264
// платежа.
//
// ЧЕМ НЕ ВЛАДЕЕТ — и это вырезано, а не оставлено «на всякий случай»:
// задачами, комментариями и логами. Их путь здесь и был источником поломки:
// `author_id` прибит в null, `created_at ?? new Date()` (отсюда комментарии с
// датой в будущем), тело клалось целиком, без разбора. В API их всё равно нет
// (GetTasks/GetComments/GetHistory → 404, проверено 16.07), так что путь был
// ещё и мёртв. Теперь ими владеют import-hollihop-exports.ts +
// communications-mappers.ts, которые разбирают выгрузку по-настоящему.
//
// ИСКЛЮЧЕНИЕ — заметка админа к занятию (`EdUnits[].Days[].Description`, 10 021
// штука). Она приходит именно отсюда, потому что живёт в дампе занятий, а не в
// выгрузке. Вешается на занятие: см. комментарий у места записи.
