...
describe('SessionService', () => {
  ...
  it('rotates a valid refresh token', async () => {
    ...
    transaction.mockImplementation(async (work) =>
      work({
        query: jest.fn().mockResolvedValueOnce({ rows: [{ id: 'session-b' }] })
      })
    );
  ...
  it('revokes session family when reuse is detected', async () => {
    ...
    expect(query).toHaveBeenCalledWith(expect.stringContaining('where family_id'), [
      'family-a'
    ]);
  });
  ...
  it('revokes all user sessions', async () => {
    ...
    expect(query).toHaveBeenCalledWith(expect.stringContaining('where user_id'), [
      'user-a'
    ]);
