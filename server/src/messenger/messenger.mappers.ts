...
export function toChatSummaryDto(row: ChatRow, opts?: { canWrite?: boolean }) {
  const ownerFullName =
...
export function toMessageDto(
  ...
  return {
    ...
    voiceDurationMs: row.deleted_at ? null : (row.voice_duration_ms ?? null),
    attachmentName: row.deleted_at
      ? null
      : (row.attachment_original_name ?? null),
    attachmentMimeType: row.deleted_at
      ? null
    ...
    attachment:
      row.deleted_at || !row.attachment_file_id
        ? null
        : {
