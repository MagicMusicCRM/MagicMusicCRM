...
describe("MessengerPolicy", () => {
  ...
  it("forbids a client from creating any direct chat", async () => {
    await expect(
      policy.canCreateDirectChat({ userId: "c", role: "client" }, "x"),
    ).rejects.toThrow(ForbiddenException);
  });
  ...
  it("forbids a direct chat whose target is a client", async () => {
    ...
    await expect(
      policy.canCreateDirectChat({ userId: "t", role: "teacher" }, "client-x"),
    ).rejects.toThrow(ForbiddenException);
  });
  ...
  it("allows a direct chat between two non-client staff/teachers", async () => {
    ...
    await expect(
      policy.canCreateDirectChat({ userId: "t", role: "teacher" }, "admin-x"),
  ...
  it("does not let an ordinary channel writer manage channel settings or ACL", () => {
    expect(() =>
      policy.assertCanManageChannel({ userId: "teacher-a", role: "teacher" }),
    ).toThrow(NotFoundException);
  });

  it("lets administration roles manage channel settings and ACL", () => {
    for (const role of [
      "admin",
      "manager",
      "director",
      ...
      expect(() =>
        policy.assertCanManageChannel({ userId: "staff-a", role }),
      ).not.toThrow();
    }
  });
