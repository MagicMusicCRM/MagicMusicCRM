...
const CHAT_CURSOR_TIMESTAMP_PATTERN =
  /^[1-9]\d{3}-(0[1-9]|1[0-2])-([0-2]\d|3[01])T([01]\d|2[0-3]):[0-5]\d:[0-5]\d\.\d{6}Z$/;
const CHAT_CURSOR_ID_PATTERN =
  /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;

function isValidChatCursorCalendarDate(value: string): boolean {
...
function isChatCursor(value: unknown): value is [string, string] {
  if (!Array.isArray(value) || value.length !== 2) return false;
  const [updatedAt, id] = value;
  if (typeof updatedAt !== "string" || typeof id !== "string") return false;
  return (
    CHAT_CURSOR_TIMESTAMP_PATTERN.test(updatedAt) &&
    isValidChatCursorCalendarDate(updatedAt) &&
    CHAT_CURSOR_ID_PATTERN.test(id)
  );
}

function decodeChatCursor(cursor: string | undefined): ChatCursor | null {
  if (!cursor) return null;
  try {
    const decoded = JSON.parse(
      Buffer.from(cursor, "base64url").toString("utf8"),
    ) as unknown;
    if (!isChatCursor(decoded)) throw new Error();
...
// Folder classification («Лиды»/«Ученики»/архив) and the owner's branch are STAFF
// inbox concepts. Non-staff queries receive null fragments so PostgreSQL can
// skip the classification work on every client's fallback poll.
const STAFF_ENTITY_FOLDER_SQL = `case
  when exists (
...
const STAFF_FOLDER_SQL = `case
  when ist.archived_at is not null then 'archive'
  else inbox_folder.entity_folder
end`;

// The student's branch wins over the stale lead's after conversion. Ordered
// link lookups keep duplicate historical links deterministic.
const STAFF_BRANCH_LATERAL_SQL = `left join lateral (
...
const NON_STAFF_BRANCH_LATERAL_SQL = `left join lateral (
  select null::text as branch_id, null::text as branch_name
) owner_branch on true`;

const CHAT_LIST_SQL_TEMPLATE = `
  ...
  left join lateral (
    select __ENTITY_FOLDER_SQL__ as entity_folder
  ) inbox_folder on true
  __BRANCH_LATERAL_SQL__
  where c.deleted_at is null
    ...
    -- archived=true returns only the actor's archived chats; the default
    -- hides archived chats. Clients have no inbox state.
    and (case when $6::boolean then ist.archived_at is not null
              else ist.archived_at is null end)
    -- Archived rows report folder='archive', while an explicit CRM folder can
    -- still narrow them by the underlying active CRM entity.
    and (
      $9::text = 'all'
      or (
...
function buildChatListSql(staffViewer: boolean): string {
  const entityFolderSql = staffViewer ? STAFF_ENTITY_FOLDER_SQL : "null::text";
  const folderSql = staffViewer ? STAFF_FOLDER_SQL : "null::text";
  const branchSql = staffViewer
    ? STAFF_BRANCH_LATERAL_SQL
    : NON_STAFF_BRANCH_LATERAL_SQL;
  return CHAT_LIST_SQL_TEMPLATE.replace("__ENTITY_FOLDER_SQL__", entityFolderSql)
    .replace("__FOLDER_SQL__", folderSql)
    .replace("__BRANCH_LATERAL_SQL__", branchSql);
}

const MESSAGE_LIST_SQL = `
    ...
    return {
      ...
      nextCursor:
        result.rows.length > limit && pageRows.length > 0
          ? encodeChatCursor(pageRows[pageRows.length - 1])
          : null,
    };
  }
