...
void main() {
  ...
  for (final role in ['admin', 'manager', 'director']) {
    testWidgets(
      ...
      (tester) async {
        final h = LiveAuditHarness(tester, role, 'account-purchase');
        ...
        for (final entity in ['student']) {
          ...
          await crm.createClientPaymentRecord(
            id,
            identity: MagicMutationIdentity.create('audit-account-funding'),
            input: CreateClientPaymentRecordInput(
              amountMinor: BigInt.from(1000000),
              status: ClientPaymentStatus.paid,
              reason: 'AUDIT-ACCOUNT-FUNDING',
              externalIdentifier: 'AUDIT-FUND-$role',
          ...
          await h.check(
            '$entity-PURCHASE',
            'Выдать абонемент с нулевой оплатой, сохранив свободный остаток отдельно',
            () async {
              await open();
              await form();
              ...
              expect(subscription.units.paid, 0);
              expect(subscription.financial.actualPaidMinor, BigInt.zero);
              expect(
          ...
          if (subscriptionId == null) {
            ...
            await h.check(
              ...
              () async {
                ...
                final balance = commerce.accounts.fold<BigInt>(
                  BigInt.zero,
                ...
                h.facts.add({
                  ...
                  'remainingObligationMinor': commerce
                      .subscriptions
                      .single
                      .financial
                      .remainingObligationMinor
                ...
                expect(balance, BigInt.from(1000000));
                expect(
                  commerce
                      .subscriptions
                      .single
