  ...
  testWidgets(
    ...
    (tester) async {
      ...
      Future<void> open({bool archived = false}) async {
        ...
        await h.quiet();
        await h.tap(find.text('Проверка OFFBOARD-STAFF'));
        await h.quiet();
        expect(detail, findsOneWidget);
        await h.tap(
          find.text(
            archived ? 'Восстановить сотрудника' : 'Отключить сотрудника',
          ),
        );
        await h.quiet();
      ...
      await h.check(
        ...
        () async {
          ...
          expect(modal(), findsNothing);
          expect((await preview())['person']['lifecycleState'], 'active');
          final staff = (await crm.listStaff(q: 'OFFBOARD-STAFF')).single;
          expect(staff['branches'], isNotEmpty);
      ...
      await h.check(
        ...
        () async {
          final result = await h.api.get<Map<String, dynamic>>(
            '/crm/staff/$id/lifecycle-history',
          );
