...
void main() {
  ...
  for (final role in ['admin', 'manager', 'director']) {
    testWidgets(
      ...
      (tester) async {
        final h = LiveAuditHarness(tester, role, 'replacement');
        ...
        for (final entity in ['student']) {
          ...
          await h.check(
            '$entity-PURCHASE',
            'Оплатить 8000 ₽ → единственный абонемент на 8 занятий',
            () async {
              await open();
              await form();
              ...
              expect(subscription.units.paid, 8);
              expect(
          ...
          if (subscriptionId != null) {
            Future<void> replacement() async {
              await open();
              await h.tap(
                find.byKey(Key('subscription-replace-$subscriptionId')),
              );
              await h.quiet();
              await h.tap(
                find.byKey(
                  Key(
                    'issue-subscription-package-${h.fixture['replacementPackageId']}',
                  ),
                ),
              );
              await h.quiet();
              expect(find.byType(SubscriptionReplacementForm), findsOneWidget);
            }

            await h.check(
              'REPLACE-PREVIEW',
              'Выбрать новый пакет и проверить пересчёт',
              () async {
                await replacement();
                expect(
                  find.byKey(const Key('subscription-replace-new')),
                  findsOneWidget,
                );
                expect(
                  (await crm.getStudentCommerceProjection(
                    studentId!,
                  )).student.subscriptions.single.status,
                  'active',
                );
              },
            );
            await h.check(
              'REPLACE-CANCEL',
              'Отмена preview сохраняет исходный абонемент',
              () async {
                await h.tap(
                  find.descendant(
                    of: find.byType(SubscriptionReplacementForm),
                    matching: find.text('Отмена'),
                  ),
                );
                await h.quiet();
                ...
                expect(
                  (await crm.getStudentCommerceProjection(
                    studentId!,
                  )).student.subscriptions.single.id,
                  subscriptionId,
                );
              },
            );
            await h.check(
              'REPLACE-VALIDATION',
              'Без подтверждения последствий замена не выполняется',
              () async {
                await replacement();
                await h.tap(
                ...
                expect(
                  find.text('Подтвердите пересчёт абонемента.'),
                  findsOneWidget,
                );
                expect(
                  (await crm.getStudentCommerceProjection(
            ...
            await h.check(
              'REPLACE',
              'Заменить пакет с сохранением исходной оплаты',
              () async {
                await h.tap(
                  find.byKey(const Key('subscription-replace-reason')),
                );
                await h.tap(find.text('Изменение условий').last);
                await h.tap(
                ...
                await h.quiet();
                final rows = (await crm.getStudentCommerceProjection(
                  studentId!,
                )).student.subscriptions;
                h.facts.add({
                  'step': h.currentStep,
                  'studentId': studentId,
                  'subscriptions': rows
                      .map((s) => s.toLegacyMap(studentId!))
                      .toList(),
                });
                final current = rows.singleWhere((s) => s.status == 'active');
                newId = current.id;
                expect(newId, isNot(subscriptionId));
                expect(current.units.total, 10);
                expect(current.financial.actualPaidMinor, BigInt.from(800000));
                expect(
                  current.financial.remainingObligationMinor,
                  BigInt.from(200000),
                );
              },
            );
            await h.check(
              'REPLACE-REOPEN',
              'Повторно открыть заменённый абонемент без повторной оплаты',
              () async {
                await open();
                final rows = (await crm.getStudentCommerceProjection(
                  studentId!,
                )).student.subscriptions;
                expect(rows.singleWhere((s) => s.status == 'active').id, newId);
                expect(
                  rows
